...
class ArtifactRecoveryTransactionTests(unittest.TestCase):
    ...
    def _build_torch_artifact(self, project_root: Path):
        from mlpipelineholder.core.models import TorchStateArtifactRecord
    ...
    def _artifact_recovery_module(self):
        return import_module("mlpipelineholder.persistence.artifact_recovery")
    ...
    def test_clone_value_rolls_back_staging_when_copy_fails(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            with patch("mlpipelineholder.persistence.artifact_recovery.shutil.copy2", side_effect=OSError("copy failed")):
                with self.assertRaises(PersistenceError):
    ...
    def test_commit_rolls_back_finalized_paths_when_rename_fails(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            with patch("mlpipelineholder.persistence.artifact_recovery._rename_path", side_effect=fail_on_second_rename):
                with self.assertRaises(PersistenceError):
                    transaction.commit()
