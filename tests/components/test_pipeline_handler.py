...
def consume_etfs_df(etfs_df: str) -> str:
    return etfs_df
...
class PipelineHandlerTests(unittest.TestCase):
    ...
    def test_atom_child_pipeline_is_atom_pipeline_subclass_after_load(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            pipeline = PipelineHandler("root", DemoConfig(base=2), project)
            pipeline.create_atom_child_pipeline(
                "atom",
                1,
                produce_seed,
            ...
            self.assertIsInstance(atom, AtomPipeline)
            self.assertIsInstance(atom, PipelineHolder)
            ...
            self.assertEqual(loaded.get_value("seed"), pipeline.get_value("seed"))
    ...
    def test_describe_pipeline_contains_blocks_functions_and_io(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("PipelineHolder(describe)", chart)
...
class StrictModeTests(unittest.TestCase):
    ...
    def test_unmapped_input_matching_sibling_declared_output_warns(self) -> None:
        with TemporaryDirectory() as temp_dir:
            tmp = Path(temp_dir)
            ...
            proc_block.register_function(consume_etfs_df, ["etfs_df"])
            log_text = (tmp / "dr" / "metadata" / "pipeline.log").read_text(
                encoding="utf-8"
            )
            self.assertIn("unmapped input", log_text)
